...
import { RequestStatus, PriceType } from '@talepo/database'
...
export default function ProviderProfilePage() {
  ...
  // Load provider-specific data when user is available
  useEffect(() => {
  ...
  }, [user, fetchServices, fetchRequests, fetchReviews])
  ...
  // Customer requests (user'ın customer olarak oluşturduğu talepler)
  const customerRequests = user.id ? requests.filter(r => r.customerId === user.id) : []
  
  // Provider requests (provider'a atanan talepler)
  const providerRequests = user.id ? requests.filter(r => r.providerId === user.id) : []

  const stats = {
    totalServices: services.length,
    activeRequests: providerRequests.filter(r => r.status === RequestStatus.PENDING || r.status === RequestStatus.ACCEPTED || r.status === RequestStatus.IN_PROGRESS).length,
    completedJobs: providerRequests.filter(r => r.status === RequestStatus.COMPLETED).length,
    rating: reviews.length > 0 ? reviews.reduce((sum, r) => sum + r.rating, 0) / reviews.length : 0,
    totalReviews: reviews.length
  }

  // Provider profile bilgileri şimdilik user'dan alınıyor
  // İleride API'den provider profile'ı getirebiliriz
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1">
            ...
            {activeTab === 'requests' && (
              <div className="bg-white rounded-2xl shadow-xl p-6">
                <h2 className="text-2xl font-bold text-gray-900 mb-6">Taleplerim</h2>
                {providerRequests.length > 0 ? (
                  <div className="space-y-4">
                    {providerRequests.map((request) => (
                      <ProviderRequestCard key={request.id} request={request as RequestWithRelations} />
                    ))}
                  </div>
                ) : (
                  <div className="text-center py-12">
                    ...
                    <p className="text-gray-600 text-lg">Henüz talep yok</p>
                  </div>
                )}
              </div>
            )}
...
// Provider Request Card Component
function ProviderRequestCard({ request }: { request: RequestWithRelations }) {
  const router = useRouter()
